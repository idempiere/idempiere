				...
				while (rsGD.next()) {
					...
					for (int i = 0; i < ncols; i++) {
						...
						if ((tableName + "_ID").equalsIgnoreCase(columnName)) {
							...
							convertTable = "";
						} else if ("Record_ID".equalsIgnoreCase(columnName) && table.columnExistsNonVirtual("AD_Table_ID")) {
							...
							if (rsGD.wasNull()) {
								...
								if (! (id == 0 && ("Parent_ID".equalsIgnoreCase(columnName) || "Node_ID".equalsIgnoreCase(columnName)))  // Parent_ID/Node_ID=0 is valid
									...
									if (convertedId < 0) {
										...
										if (convertedId < 0) {
											if (("Record_ID".equalsIgnoreCase(columnName) && table.columnExistsNonVirtual("AD_Table_ID"))
