...
public class MTreeFavorite extends X_AD_Tree_Favorite
{
	...
	public static final String				SQL_GET_TREE_FAVORITE_ID	= "SELECT AD_Tree_Favorite_ID FROM AD_Tree_Favorite	WHERE IsActive = 'Y' AND AD_Client_ID = ? AND AD_Role_ID = ? AND AD_User_ID = ?";
	...
	private static CCache<String, Integer>	cache_TreeFavID				= new CCache<String, Integer>("Cache_AD_Tree_Favorite_ID", 7 * 24 * 60);
	 ...
	 */
	public static int getFavoriteTreeID(int clientID, int roleID, int userID)
	{
		String key = "ID_" + clientID + "_" + roleID + "_" + userID;
