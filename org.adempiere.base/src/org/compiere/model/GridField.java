	...
	protected Object defaultFromExpression (){
		...
		if (m_vo.DefaultValue != null && !m_vo.DefaultValue.equals("") && !m_vo.DefaultValue.startsWith(MColumn.VIRTUAL_UI_COLUMN_PREFIX))
		{
			...
			while (st.hasMoreTokens())
			{
				...
				else if (DisplayType.isChosenMultipleSelection(m_vo.displayType)) {
					defStrMultipleSelec += defStr + ",";
					if (!st.hasMoreTokens()) {
						defStr = defStrMultipleSelec.substring(0, defStrMultipleSelec.length() - 1);
					} else {
						continue;
					}
				}
