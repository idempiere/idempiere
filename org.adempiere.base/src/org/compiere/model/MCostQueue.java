...
public class MCostQueue extends X_M_CostQueue
{
			...
			IAcctSchemaModel as, int AD_Org_ID, int M_CostElement_ID, String trxName)
	{
		...
		try
		{
			...
			pstmt.setInt (5, as.getAcctSchema().getM_CostType_ID());
	...
	public static MCostQueue[] getQueue (MProduct product, int M_ASI_ID,
			IAcctSchemaModel as, int Org_ID, MCostElement ce, String trxName)
	{
		...
		try
		{
			...
			pstmt.setInt (3, product.getM_Product_ID());
			pstmt.setInt (4, as.getAcctSchema().getM_CostType_ID());
			pstmt.setInt (5, as.getAcctSchema().getC_AcctSchema_ID());
	...
	public static BigDecimal adjustQty (MProduct product, int M_ASI_ID,
			IAcctSchemaModel as, int Org_ID, MCostElement ce, BigDecimal Qty, 
		String trxName)
	{
		if (Qty.signum() == 0)
			return Env.ZERO;
	...
	public static BigDecimal getCosts (MProduct product, int M_ASI_ID,
			org.adempiere.base.acct.model.IAcctSchemaModel as, int Org_ID, MCostElement ce, BigDecimal Qty, 
		String trxName)
	{
		if (Qty.signum() == 0)
			return Env.ZERO;
