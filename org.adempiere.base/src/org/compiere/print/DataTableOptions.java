...
import org.compiere.print.ReportEngine.FunctionTypes;
...
//		dataOptions.append(", fixedHeader: ").append(true);
		...
		if(groupByDataSrc != null) {
			...
			if(!Util.isEmpty(aggregateFunctions)) {
				...
				//	.append("  ");
				//.append(" function ( rows, group ) { return $('<tr/>').append('<td colspan=" + printColLastIndex+1 + " />'); } ");
			}
			dataOptions.append(" } ");
		}
...
//		}
	...
	/**
	 * 
	 * @param isFinalAggregateRow
	 * @return
	 * @deprecated
	 */
	public String getAggregateFunctions(boolean isFinalAggregateRow ) {
		...
		if(isFinalAggregateRow)
			...
			retValue.append(" = group; ");
		groupRowNamesMap.put(groupBy, groupName);			

		
		retValue.append(" let totalRows = []; ");
		if(isFinalAggregateRow) {
			retValue.append(" totalRows = data; ");
		} else {
			retValue.append(" rows.rows().data().filter(function(value, index) { ")
			.append(" let groupValue = null; ")		
			.append(" if(value[").append(groupBy).append("].includes('<a ')) { ")
		...
		if(averageList != null) {
			for(Integer averageID:averageList) {
				String varName = "average_"+averageID;
				groupRowNamesMap.put(averageID, varName);
				retValue.append(" let ").append(varName).append(" =  new Intl.NumberFormat(locale).format(+(( totalRows.map(i => new NumberParser(locale).parse(i[").append(averageID).append("])).reduce( function (a, b) { return a + b*1; }, 0) ");
				retValue.append(" / totalRows.length ).toFixed(4))); ");
//				number.toLocaleString('en-US', { style: 'currency', currency: 'USD' })
		...
		if(countList != null) {
			for(Integer countID:countList) {
				String varName = "count_"+countID;
				groupRowNamesMap.put(countID, varName);
				retValue.append(" let ").append(varName).append(" =  new Intl.NumberFormat(locale).format(totalRows.length); ");
			}
		}
		...
		if(sumList != null) {
			for(Integer sumID:sumList) {
				String varName = "sum_"+sumID;
				groupRowNamesMap.put(sumID, varName);
				retValue.append(" let ").append(varName).append(" =  new Intl.NumberFormat(locale).format(+((totalRows.map(i => new NumberParser(locale).parse(i[").append(sumID).append("])).reduce( function (a, b) { return a + b*1; }, 0)).toFixed(4))); ");
			}
		}
		...
		if(maxList != null) {
			for(Integer maxID:maxList) {
				String varName = "max_"+maxID;
				groupRowNamesMap.put(maxID, varName);
				retValue.append(" let ").append(varName).append(" =  new Intl.NumberFormat(locale).format(totalRows.map(i => new NumberParser(locale).parse(i[").append(maxID)
					.append("])).reduce(function(a, b) { return Math.max(a, b);})); ");
			}
		}
		...
		if(minList != null) {
			for(Integer minID:minList) {
				String varName = "min_"+minID;
				groupRowNamesMap.put(minID, varName);
				retValue.append(" let ").append(varName).append(" =  new Intl.NumberFormat(locale).format( totalRows.map(i => new NumberParser(locale).parse(i[").append(minID)
				.append("])).reduce(function(a, b) { return Math.min(a, b);})); ");
			}
		}
		...
		if(varianceList != null) {
			for(Integer varianceID:varianceList) {
				String varName = "variance_"+varianceID;
				groupRowNamesMap.put(varianceID, varName);
				
				retValue.append(" let ").append(varName).append(" = totalRows.map((i) => { return { sum: new NumberParser(locale).parse(i[").append(varianceID)
					.append("]), sumSquare: (new NumberParser(locale).parse(i[").append(varianceID).append("]) * new NumberParser(locale).parse(i[").append(varianceID).append("]))  ")
					.append(" }; } )");
				retValue.append(".reduce( function (a, b) { let c = { sum: a.sum + b.sum*1, sumSquare: a.sumSquare + b.sumSquare*1 }; return c; }, { sum: 0, sumSquare: 0 }); ");
				
				retValue.append(varName).append(" = new Intl.NumberFormat(locale).format( +(((").append(varName).append(".sumSquare - (( ").append(varName).append(".sum * ").append(varName).append(".sum ) / totalRows.length ))/ totalRows.length ).toFixed(4))); ");
			}
		}
		...
		if(deviationList != null) {
			for(Integer deviationID:deviationList) {
				String varName = "deviation_"+deviationID;
				groupRowNamesMap.put(deviationID, varName);
				
				retValue.append(" let ").append(varName).append(" = totalRows.map((i) => { return { sum: new NumberParser(locale).parse(i[").append(deviationID)
					.append("]), sumSquare: (new NumberParser(locale).parse(i[").append(deviationID).append("]) * new NumberParser(locale).parse(i[").append(deviationID).append("]))  ")
					.append(" }; } )");
				retValue.append(".reduce( function (a, b) { let c = { sum: a.sum + b.sum*1, sumSquare: a.sumSquare + b.sumSquare*1 }; return c; }, { sum: 0, sumSquare: 0 }); ");
				
				retValue.append(varName).append(" = ((").append(varName).append(".sumSquare - (( ").append(varName).append(".sum * ").append(varName).append(".sum ) / totalRows.length ))/ totalRows.length ); ");
				retValue.append(varName).append(" =  new Intl.NumberFormat(locale).format(+(Math.sqrt(").append(varName).append(").toFixed(4)));");
			}
		}
		...
		for(Entry<Integer, String> groupRowNameSet :groupRowNamesMap.entrySet()) {
			Integer actualColIndex = groupRowNameSet.getKey();
			String actualValue = groupRowNameSet.getValue();
			...
			if(actualValue.contains("groupBy_"))
				retValue.append(".append( '<td class=\"datatable-text\" >  ");
			else
				retValue.append(".append( '<td class=\"datatable-number\" > ");
			if(!Util.isEmpty(actualValue)) {
				retValue.append("'+"+actualValue+"+'");
			}
			...
			retValue.append("</td>' )");
			prevColIndex = Util.isEmpty(actualValue) ? actualColIndex : actualColIndex+1;
		...
		retValue.append("; ");
		if(isFinalAggregateRow)
			retValue.append(" $(tfoot).append(rowRender);");
		else
			retValue.append("return rowRender;");
