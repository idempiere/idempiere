...
public class Doc_BankStatement extends Doc
{
	 ...
	 * 	@param trxName trx
	 */
	public Doc_BankStatement (IAcctSchemaModel as, ResultSet rs, String trxName)
	{
	...
	public ArrayList<Fact> createFacts (IAcctSchemaModel as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			IAccountModel acct_bank_asset =  getAccount(Doc.ACCTTYPE_BankAsset, as);
			IAccountModel acct_bank_in_transit = getAccount(Doc.ACCTTYPE_BankInTransit, as);

			// don't validate interorg on banks for this - normally banks are balanced by orgs
			if ((!as.getAcctSchema().isPostIfClearingEqual()) && acct_bank_asset.equals(acct_bank_in_transit)) {
				...
				if (amt_stmt_minus_trx.compareTo(Env.ZERO) != 0) {
					...
					if (fl != null && AD_Org_ID != 0)
						fl.getFactAcctInfo().getFactAcct().setAD_Org_ID(AD_Org_ID);
					if (fl != null && C_BPartner_ID != 0)
						fl.getFactAcctInfo().getFactAcct().setC_BPartner_ID(C_BPartner_ID);
				...
				if (statementLine.getC_DepositBatch_ID() != 0) {
					...
					for (MDepositBatchLine depositLine : depositBatchLines) {
						...
						// line id
						fl.getFactAcctInfo().getFactAcct().setLine_ID(statementLine.get_ID());
						if (fl != null) {
							if (C_BPartner_ID != 0)
								fl.getFactAcctInfo().getFactAcct().setC_BPartner_ID(C_BPartner_ID);
							if (AD_Org_ID != 0)
								...
								fl.getFactAcctInfo().getFactAcct().setAD_Org_ID(docDepositLine.getAD_Org_ID(true)); // from payment
						}
					}
				} else {
					...
					if (fl != null) {
						...
						if (AD_Org_ID != 0)
							...
							fl.getFactAcctInfo().getFactAcct().setAD_Org_ID(line.getAD_Org_ID(true)); // from payment
					}
				}
