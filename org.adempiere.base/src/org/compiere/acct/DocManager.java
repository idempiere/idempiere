...
public class DocManager {
	 ...
	 */
	public static Doc getDocument(IAcctSchemaModel as, int AD_Table_ID, int Record_ID, String trxName)
	{
		...
		String cacheKey = as.getAcctSchema().getC_AcctSchema_ID() + "|" + AD_Table_ID;
		...
		query.put("gaap", as.getAcctSchema().getGAAP());
	 ...
	 */
	public static Doc getDocument(IAcctSchemaModel as, int AD_Table_ID, ResultSet rs, String trxName)
	{
		String cacheKey = as.getAcctSchema().getC_AcctSchema_ID() + "|" + AD_Table_ID;
		IServiceReferenceHolder<IDocFactory> cache = s_DocFactoryCache.get(cacheKey);
		if (cache != null)
	 ...
	 */
	public static String postDocument(IAcctSchemaModel[] ass,
	 ...
	 * @return null if the document was posted or error message
	 */
	protected static String postDocument(IAcctSchemaModel[] ass,
	 ...
	 * @return null if the document was posted or error message
	 */
	private static String postDocument(IAcctSchemaModel[] ass,
	 ...
	 */
	private static String startBackDateProcess(IAcctSchemaModel[] ass, int AD_Table_ID, int Record_ID, String trxName) {
		if (ass.length == 0)
			return null;
		...
		for (int x = 0; x < ass.length; x++)
			whereClause.append((x > 0) ? "," : "").append(ass[x].getAcctSchema().getC_AcctSchema_ID());	
		whereClause.append(") ");
		...
		for (int x = 0; x < ass.length; x++)
			selectSql.append((x > 0) ? "," : "").append(ass[x].getAcctSchema().getC_AcctSchema_ID());	
		selectSql.append(") ");
