...
public class Doc_Inventory extends Doc
{
	 ...
	 * 	@param trxName trx
	 */
	public Doc_Inventory (IAcctSchemaModel as, ResultSet rs, String trxName)
	{
	...
	{		
		...
		if (MDocType.DOCSUBTYPEINV_CostAdjustment.equals(parentDocSubTypeInv))
		{
			if (inventory.getC_Currency_ID() == 0)
				setC_Currency_ID(MClient.get(getCtx()).getAcctSchema().getAcctSchema().getC_Currency_ID()); 
		} else 
		{
	...
	{		
		...
		if (MDocType.DOCSUBTYPEINV_CostAdjustment.equals(dt.getDocSubTypeInv())) {
			...
			if (getAcctSchema().getAcctSchema().getC_AcctSchema_ID() == as.getAcctSchema().getC_AcctSchema_ID()) {
...
//				continue;
	...
	public ArrayList<Fact> createFacts (IAcctSchemaModel as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (doPosting)
			{		
				int C_Currency_ID = getC_Currency_ID() > 0 ? getC_Currency_ID() : as.getAcctSchema().getC_Currency_ID();
				//  Inventory       DR      CR
				...
				if (dr != null)
				{
					...
					if (cr != null)
					{
						...
						if (line.getC_Charge_ID() != 0)	//	explicit overwrite for charge
							cr.getFactAcctInfo().getFactAcct().setAD_Org_ID(line.getAD_Org_ID());
			...
			if (doPosting || costAdjustment)
			{
				...
				if (costAdjustment && getC_Currency_ID() > 0 && getC_Currency_ID() != as.getAcctSchema().getC_Currency_ID()) 
				{
					costDetailAmt = MConversionRate.convert (getCtx(),
							costDetailAmt, getC_Currency_ID(), as.getAcctSchema().getC_Currency_ID(),
