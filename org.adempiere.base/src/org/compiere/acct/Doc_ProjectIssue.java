...
public class Doc_ProjectIssue extends Doc
{
	 ...
	 * 	@param trxName trx
	 */
	public Doc_ProjectIssue (IAcctSchemaModel as, ResultSet rs, String trxName)
	{
	...
	public ArrayList<Fact> createFacts (IAcctSchemaModel as)
	{
		...
		dr = fact.createLine(m_line,
			getAccount(acctType, as), as.getAcctSchema().getC_Currency_ID(), cost, null);
		dr.getFactAcctInfo().getFactAcct().setQty(m_line.getQty().negate());
		...
		cr = fact.createLine(m_line,
			...
			as.getAcctSchema().getC_Currency_ID(), null, cost);
