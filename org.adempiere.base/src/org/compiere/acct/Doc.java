...
public abstract class Doc
{
	 ...
	 */
	public static Doc get (IAcctSchemaModel as, int AD_Table_ID, int Record_ID, String trxName)
	{
	 ...
	 */
	public static Doc get (IAcctSchemaModel as, int AD_Table_ID, ResultSet rs, String trxName)
	{
	 ...
	 */
	public static String postImmediate (IAcctSchemaModel[] ass,
	 ...
	 * 	@param trxName trx
	 */
	public Doc (IAcctSchemaModel as, Class<?> clazz, ResultSet rs, String defaultDocumentType, String trxName)
	{
		p_Status = STATUS_Error;
		m_as = as;
		m_ctx = new Properties(m_as.getPO().getCtx());
		m_ctx.setProperty(Env.AD_CLIENT_ID, String.valueOf(m_as.getAcctSchema().getAD_Client_ID()));
	...
	public final String post (boolean force, boolean repost, boolean isInBackDatePostingProcess)
	{
		...
		//
		if (p_po.getAD_Client_ID() != m_as.getAcctSchema().getAD_Client_ID())
		{
			StringBuilder error = new StringBuilder("AD_Client_ID Conflict - Document=").append(p_po.getAD_Client_ID())
				.append(", AcctSchema=").append(m_as.getAcctSchema().getAD_Client_ID());
		...
		try
		{
			...
			boolean skip = false;
			if (m_as.getAcctSchema().getAD_OrgOnly_ID() != 0)
			{
				//	Header Level Org
		...
		if (!p_Status.equals(STATUS_Posted) && !p_Status.equals(STATUS_Deferred))
		{
			...
			.append(" - " + Msg.getElement(Env.getCtx(),"C_AcctSchema_ID") + "=").append(m_as.getAcctSchema().getName());
	...
	{				
		...
		int no = DB.executeUpdate(sql.toString(), new Object[] {get_Table_ID(), p_po.get_ID(), m_as.getAcctSchema().getC_AcctSchema_ID()}, false, getTrxName());
		if (no != 0)
		...
		no = DB.executeUpdate(sql.toString(), 
				new Object[] {new Timestamp(System.currentTimeMillis()), get_Table_ID(), p_po.get_ID(), m_as.getAcctSchema().getC_AcctSchema_ID()}, 
				false, getTrxName());
		if (no != 0)
		...
		sql = new StringBuilder ("DELETE FROM Fact_Acct ")
			...
			.append(" AND Record_ID=?")
			.append(" AND C_AcctSchema_ID=?");
		no = DB.executeUpdate(sql.toString(), new Object[] {get_Table_ID(), p_po.get_ID(), m_as.getAcctSchema().getC_AcctSchema_ID()}, false, getTrxName());
		if (no != 0)
	...
	public boolean isConvertible (IAcctSchemaModel acctSchema)
	{
		...
		if (this instanceof Doc_GLJournal) {
			...
			if (acctSchema.getAcctSchema().getC_AcctSchema_ID() != glj_as)
				return true;
		}
		//  Get All Currencies
		...
		if (set.size() == 1 && acctSchema.getAcctSchema().getC_Currency_ID() == getC_Currency_ID())
		{
		...
		while (it.hasNext() && convertible)
		{
			...
			if (C_Currency_ID != acctSchema.getAcctSchema().getC_Currency_ID())
			{
				BigDecimal amt = MConversionRate.getRate (C_Currency_ID, acctSchema.getAcctSchema().getC_Currency_ID(),
					getDateAcct(), getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
				if (amt == null)
				{
					convertible = false;
					log.warning ("NOT from C_Currency_ID=" + C_Currency_ID
						+ " to " + acctSchema.getAcctSchema().getC_Currency_ID()
						+ " - " + toString());
				}
				else
		...
		if (log.isLoggable(Level.FINE)) log.fine("Convertible=" + convertible + ", AcctSchema C_Currency_ID=" + acctSchema.getAcctSchema().getC_Currency_ID() + " - " + toString());
		return convertible;
	}	//	isConvertible
	 ...
	 */
	public int getValidCombination_ID (int AcctType, IAcctSchemaModel as)
	{
		...
		try
		{
			...
			if (para_1 == -1)   //  GL Accounts
				pstmt.setInt (1, as.getAcctSchema().getC_AcctSchema_ID());
			else
			{
	 ...
	 *  @return MAccount
	 */
	public final IAccountModel getAccount (int AcctType, IAcctSchemaModel as)
	{
	 ...
	 *  @return Facts
	 */
	public abstract ArrayList<Fact> createFacts (IAcctSchemaModel as);
	 ...
	 * @return MAcctSchema
	 */
	protected IAcctSchemaModel getAcctSchema() {
		return m_as;
	}
