...
public class InventoryValue extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
	  ...
	  IAcctSchemaModel as = c.getAcctSchema();
	  String msg = "";
	  ...
	  for (int l_M_Warehouse_ID: p_M_Warehouse_IDs) {
		...
		if (as.getAcctSchema().getC_Currency_ID() != p_C_Currency_ID)
		{
			final String sql11 = "UPDATE T_InventoryValue iv "
				...
				+ "	Cost= "
					...
					+ "FROM C_AcctSchema acs WHERE acs.C_AcctSchema_ID=?) "
				+ "WHERE iv.M_Warehouse_ID=? AND iv.AD_PInstance_ID=?";
			no = DB.executeUpdateEx (sql11, new Object[] {as.getAcctSchema().getC_AcctSchema_ID(), as.getAcctSchema().getC_AcctSchema_ID(), l_M_Warehouse_ID, getAD_PInstance_ID()}, get_TrxName());
