...
public class FactAcctReset extends SvrProcess
{
	...
	private void delete (String TableName, int AD_Table_ID)
	{
		...
		boolean autoPeriod = as != null && as.getAcctSchema().isAutoPeriodControl();
		if (autoPeriod)
		{
			Timestamp temp = TimeUtil.addDays(today, - as.getAcctSchema().getPeriod_OpenHistory());
			...
			temp = TimeUtil.addDays(today, as.getAcctSchema().getPeriod_OpenFuture());
