...
public class ProjectIssue extends SvrProcess
{
	...
	private String issueInvoiceLine()
	{
		...
		if (!(MInvoice.DOCSTATUS_Completed.equals(inv.getDocStatus())
				|| MInvoice.DOCSTATUS_Closed.equals(inv.getDocStatus())))
			throw new IllegalArgumentException(Msg.getMsg(getCtx(), "InvoiceLineMandatoryCOCL", new Object[] { invLine }));
		...
		if (MDocType.DOCBASETYPE_APCreditMemo.equals((inv.getDocBaseType())))
		...
		else if (!Util.isEmpty(inv.getDescription()))
			pi.setDescription(inv.getDescription());
