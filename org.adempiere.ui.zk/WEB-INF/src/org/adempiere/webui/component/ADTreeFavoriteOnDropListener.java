...
public class ADTreeFavoriteOnDropListener implements EventListener<Event>
{
	...
	private void menuItemList(DefaultTreeNode<Object> toNode)
	{
		...
		if (mtn.isSummary())
		{
			...
			else
			{
				...
				if (ThemeManager.isUseFontIconForImage())
					menuItem.setIconSclass("z-icon-Collapse");
				else
	...
	class DeleteListener implements EventListener<Event>
	{
		...
		private void deleteNodeItem(DefaultTreeNode<Object> toNode)
		{
			...
			MTreeFavoriteNode mtfNode = (MTreeFavoriteNode) MTable.get(Env.getCtx(), MTreeFavoriteNode.Table_ID).getPO(nodeID, null);
	...
	{
		MTreeFavoriteNode mtFavNode = (MTreeFavoriteNode) MTable.get(Env.getCtx(), MTreeFavoriteNode.Table_ID).getPO(0, null);
