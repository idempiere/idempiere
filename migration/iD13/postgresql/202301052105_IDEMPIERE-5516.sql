...
-- Jan 5, 2023, 9:05:26 PM IST
ALTER TABLE C_BP_Customer_Acct ADD COLUMN B_UnallocatedCash_Acct NUMERIC(10) DEFAULT NULL
;
...
-- Jan 5, 2023, 9:11:56 PM IST
ALTER TABLE C_BP_Group_Acct ADD COLUMN B_UnallocatedCash_Acct NUMERIC(10) DEFAULT NULL 
;
...
INSERT INTO t_alter_column values('c_bp_group_acct','B_UnallocatedCash_Acct','NUMERIC(10)',null,null)
;
...
-- Jan 5, 2023, 9:13:50 PM IST
ALTER TABLE C_BP_Group_Acct ADD COLUMN B_PaymentSelect_Acct NUMERIC(10) DEFAULT NULL 
;
...
-- Jan 5, 2023, 9:15:24 PM IST
ALTER TABLE C_BP_Vendor_Acct ADD COLUMN B_PaymentSelect_Acct NUMERIC(10) DEFAULT NULL 
;
...
-- CHECK CLIENT AGAINST DUPLICATE ELEMENT ACCOUNT USED IN BANK ACCOUNT ACCOUNTING
...
BEGIN
	...
	IF duplicateAcctFound THEN
		RAISE WARNING 'Bank account found different UnAllocatedCash_Acct used against same client under same acctSchema, Please verify it [ IDEMPIERE-5516 ].';
	ELSE
		-- Update C_BP_Group_Acct > B_UnallocatedCash_Acct from Bank Acct
		UPDATE 	C_BP_Group_Acct
		SET 	B_UnallocatedCash_Acct = a.B_UnallocatedCash_Acct
		FROM 	C_AcctSchema_Default a
		WHERE  	a.IsActive = 'Y' 
				AND C_BP_Group_Acct.C_AcctSchema_ID = a.C_AcctSchema_ID 
				AND C_BP_Group_Acct.AD_Client_ID = a.AD_Client_ID
				AND C_BP_Group_Acct.B_UnallocatedCash_Acct IS NULL
		;
		...
		WHERE 		C_BP_Customer_Acct.AD_Client_ID = a.AD_Client_ID 
				AND C_BP_Customer_Acct.C_AcctSchema_ID  = a.C_AcctSchema_ID 
				AND C_BP_Customer_Acct.C_BPartner_ID = b.C_BPartner_ID
				AND C_BP_Customer_Acct.B_UnallocatedCash_Acct IS NULL
		;
...
BEGIN
	...
	IF duplicateAcctFound THEN
		RAISE WARNING 'Bank account found different PaymentSelect_Acct used against same client under same acctSchema, Please verify it [ IDEMPIERE-5516 ].';
	ELSE
		-- Update C_BP_Group_Acct > B_PaymentSelect_Acct from Bank Acct
		UPDATE 	C_BP_Group_Acct
		SET 	B_PaymentSelect_Acct = a.B_PaymentSelect_Acct
		FROM 	C_AcctSchema_Default a
		WHERE  	a.IsActive = 'Y' 
				AND C_BP_Group_Acct.C_AcctSchema_ID = a.C_AcctSchema_ID 
				AND C_BP_Group_Acct.AD_Client_ID = a.AD_Client_ID
				AND C_BP_Group_Acct.B_PaymentSelect_Acct IS NULL
		;
		...
		FROM 	C_BP_Group_Acct a
		INNER JOIN C_BPartner b ON (b.C_BP_Group_ID = a.C_BP_Group_ID)
		WHERE 		C_BP_Vendor_Acct.AD_Client_ID = a.AD_Client_ID 
				AND C_BP_Vendor_Acct.C_AcctSchema_ID  = a.C_AcctSchema_ID 
				AND C_BP_Vendor_Acct.C_BPartner_ID = b.C_BPartner_ID
				AND C_BP_Vendor_Acct.B_PaymentSelect_Acct IS NULL
		;
