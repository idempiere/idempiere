...
{	
	...
	public int createElementValue (String value, String name, boolean isExpenseType)
	{
		...
		ev.getElementValue().setAD_Org_ID(m_AD_Org_ID);
		ev.getElementValue().setC_Element_ID(m_C_Element_ID);
		try {
			ev.getPO().saveEx();
		} catch (Exception e) {
		...
		return ev.getElementValue().getC_ElementValue_ID();
	}   //  createElementValue
    ...
    public int createCharge(String name, int elementValueId)
    {
        ...
        charge.setName(account.getAccountModel().getElementValue().getValue() + " " + name);
     ...
     */
    private void updateAccount(MCharge charge, IAccountModel account)
    {
    ...
    private boolean isAccountSchemaValid()
    {
        ...
        else if (m_acctSchema.getAcctSchema().getC_AcctSchema_ID() == 0)
        {
            return false;
        }
     ...
     */
    private StringBuffer createUpdateAccountSql(MCharge charge, IAccountModel account)
    {
        StringBuffer sql = new StringBuffer("UPDATE C_Charge_Acct ");
        sql.append("SET CH_Expense_Acct=").append(account.getCombination().getC_ValidCombination_ID());
    ...
    private IAccountModel getAccount(int elementValueId)
    {
    	...
    	IAccountModel account = AcctInfoServices.getAccountInfoService().get(ctx,
            ...
            elementValueId,
            defaultAccount.getCombination().getC_SubAcct_ID(),
            defaultAccount.getCombination().getM_Product_ID(),
            defaultAccount.getCombination().getC_BPartner_ID(),
            defaultAccount.getCombination().getAD_OrgTrx_ID(),
            defaultAccount.getCombination().getC_LocFrom_ID(),
            defaultAccount.getCombination().getC_LocTo_ID(),
            defaultAccount.getCombination().getC_SalesRegion_ID(),
            defaultAccount.getCombination().getC_Project_ID(),
            defaultAccount.getCombination().getC_Campaign_ID(),
            defaultAccount.getCombination().getC_Activity_ID(),
            defaultAccount.getCombination().getUser1_ID(),
            defaultAccount.getCombination().getUser2_ID(),
            defaultAccount.getCombination().getUserElement1_ID(),
            defaultAccount.getCombination().getUserElement2_ID(),
            m_trxName);
